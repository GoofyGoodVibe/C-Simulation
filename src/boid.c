...
void update_boid(Boid *boid, Flock *flock, ObstacleManager *obstacle_mgr) {
    ...
    seperation_force = calculate_seperation(boid, flock);
    ...
    total_force = vec_add(total_force, vec_mult(seperation_force, Boid_Seperation_Weight));
...
Vector2D calculate_boundaries(Boid *boid) {
    ...
    if (near_edge) {
        ...
        steering = vec_limit(steering, Boid_Max_Force * 1.5f); 
    }
